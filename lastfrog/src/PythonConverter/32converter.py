...
img = Image.open("carredr.png")  # Change to your actual image path
...
for i in range(1024):
    if i < len(bram_array):
        val = bram_array[i]
        print(f"red_car_right_bram[{i}] = 6'b{val:06b};")
    else:
        print(f"red_car_right_bram[{i}] = 6'b000000;")  # Fallback for any missing pixels
